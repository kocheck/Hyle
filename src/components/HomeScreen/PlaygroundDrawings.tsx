...
// Drawing Primitives helpers - Moved outside
const DrawTree = ({ x, y }: {x: number, y: number}) => {
...
const DrawRock = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
...
const DrawDoor = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
    <Group>
...
const DrawCrown = ({ x, y, size }: {x: number, y: number, size: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return <Line points={[
...
const DrawSkull = ({ x, y, size }: {x: number, y: number, size: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
    <Group>
...
const DrawZzz = ({ x, y, size }: {x: number, y: number, size: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
    <Group>
...
const DrawChest = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
    <Group>
       {/* Box */}
...
const DrawSparkles = ({ x, y, size }: {x: number, y: number, size: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
    <Group>
...
const DrawQuestion = ({ x, y, size }: {x: number, y: number, size: number}) => {
   const color = useMemo(() => getRandomColor(), []);
   return (
     <Group>
...
const DrawExclamation = ({ x, y, size }: {x: number, y: number, size: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
    <Group>
...
const DrawTracks = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
    <Group>
...
const DrawSword = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
      <Group>
          {/* Blade */}
...
const DrawPotion = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
      <Group>
          {/* Bottle */}
...
const DrawScroll = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
      <Group>
...
const DrawFire = ({ x, y }: {x: number, y: number}) => {
  const color = useMemo(() => getRandomColor(), []);
  return (
...
export const PlaygroundDrawings = memo(function PlaygroundDrawings({ tokens = [] }: PlaygroundDrawingsProps) {
  ...
  // Randomize which props appear for this session
  ...
  const activeProps = useMemo(() => {
    ...
    return {
        ranger: pick([{id: 'tree', weight: 40}, {id: 'tracks', weight: 30}, {id: 'potion', weight: 20}, {id: 'none', weight: 10}]),
        goblin: pick([{id: 'rock', weight: 30}, {id: 'zzz', weight: 20}, {id: 'skull', weight: 20}, {id: 'trap', weight: 20}, {id: 'none', weight: 10}]),
        hero:   pick([{id: 'door', weight: 30}, {id: 'chest', weight: 20}, {id: 'sword', weight: 20}, {id: 'scroll', weight: 20}, {id: 'none', weight: 10}]),
        wizard: pick([{id: 'sparkles', weight: 30}, {id: 'question', weight: 20}, {id: 'fire', weight: 30}, {id: 'potion', weight: 20}]),
        dragon: pick([{id: 'crown', weight: 40}, {id: 'fire', weight: 30}, {id: 'chest', weight: 20}, {id: 'zzz', weight: 10}]),
    };
  }, []); // Run once on mount

  return (
    <>
      {activeProps.ranger === 'tree' && <DrawTree x={ranger.x} y={ranger.y} />}
      {activeProps.ranger === 'tracks' && <DrawTracks x={ranger.x} y={ranger.y} />}
      {activeProps.ranger === 'potion' && <DrawPotion x={ranger.x + 40} y={ranger.y} />}

      {activeProps.goblin === 'rock' && <DrawRock x={goblin.x} y={goblin.y} />}
      {activeProps.goblin === 'zzz' && <DrawZzz x={goblin.x} y={goblin.y} size={goblin.size} />}
      {activeProps.goblin === 'skull' && <DrawSkull x={goblin.x} y={goblin.y} size={goblin.size} />}
      {/* Trap reused as skull for now or generic danger? Let's use Skull for trap actually or drawing spikes is easy */}
      {activeProps.goblin === 'trap' && <DrawSkull x={goblin.x + 30} y={goblin.y + 30} size={goblin.size} />}

      {activeProps.hero === 'door' && <DrawDoor x={hero.x} y={hero.y} />}
      {activeProps.hero === 'chest' && <DrawChest x={hero.x + 60} y={hero.y} />}
      {activeProps.hero === 'exclamation' && <DrawExclamation x={hero.x} y={hero.y} size={hero.size} />}
      {activeProps.hero === 'sword' && <DrawSword x={hero.x - 30} y={hero.y} />}
      {activeProps.hero === 'scroll' && <DrawScroll x={hero.x + 30} y={hero.y + 30} />}

      {activeProps.wizard === 'sparkles' && <DrawSparkles x={wizard.x} y={wizard.y} size={wizard.size} />}
      {activeProps.wizard === 'question' && <DrawQuestion x={wizard.x} y={wizard.y} size={wizard.size} />}
      {activeProps.wizard === 'skull' && <DrawSkull x={wizard.x} y={wizard.y} size={wizard.size} />}
      {activeProps.wizard === 'fire' && <DrawFire x={wizard.x + 30} y={wizard.y - 30} />}
      {activeProps.wizard === 'potion' && <DrawPotion x={wizard.x - 20} y={wizard.y + 20} />}

      {activeProps.dragon === 'crown' && <DrawCrown x={dragon.x} y={dragon.y} size={dragon.size} />}
      {activeProps.dragon === 'zzz' && <DrawZzz x={dragon.x} y={dragon.y} size={dragon.size} />}
      {activeProps.dragon === 'chest' && <DrawChest x={dragon.x + 80} y={dragon.y + 20} />}
      {activeProps.dragon === 'fire' && <DrawFire x={dragon.x - 50} y={dragon.y + 50} />}
    </>
  );
});
